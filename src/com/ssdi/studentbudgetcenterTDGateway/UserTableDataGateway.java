...
public class UserTableDataGateway {
		...
		try {
			...
			if (result) {
				preparedstatement2 = conn
						.prepareStatement("select sum(transactionAmount) from Transaction where username='"
								+ userId + "'");
				ResultSet rs1 = preparedstatement2.executeQuery();
				preparedstatement3 = conn
						.prepareStatement("select sum(BudgetTarget) from BudgetItems where username='"
								+ userId + "'");
				ResultSet rs2 = preparedstatement3.executeQuery();
				while (rs1.next() && rs2.next()) {
					String mail = "";
					Double ratio = (rs1.getDouble(1)) / (rs2.getDouble(1));
					preparedstatement4 = conn
							.prepareStatement("select email from user where username='"
									+ userId + "'");
					...
					if (ratio >= 0.75 && ratio < 1) {
						...
						preparedstatement5 = conn
								.prepareStatement("update user set Emailcount=1 where username='"
										+ userId + "'");
						preparedstatement5.execute();
					} else if (ratio >= 1) {
						sendEmail("You have consumed 100% of your Budget", mail);
						preparedstatement6 = conn
								.prepareStatement("update user set Emailcount=2 where username='"
										+ userId + "'");
		...
		try {
			...
			preparedstatement1 = conn
					.prepareStatement("insert into user values('"
							...
							+ "'," + user.getPasswordseed() + ",'"
							+ user.getEmail() + "'," + user.getEmailcount()
							+ ");");
			preparedstatement1.execute();
			result1 = true;
			preparedstatement2 = conn
					.prepareStatement("update BankAccount set username='"
							+ user.getUsername() + "';");
				...
				new javax.mail.Authenticator() {
					protected PasswordAuthentication getPasswordAuthentication() {
						return new PasswordAuthentication(
								"udiptaroy@gmail.com", "focus123#");// Specify
																	// the
																	// Username
																	// and the
																	// PassWord
		...
		try {
			...
			preparedstatement1 = conn
					.prepareStatement("update user set Password='"
							+ user1.getPassword() + "', PasswordSeed="
							+ user1.getPasswordseed() + ", Email='"
							+ user1.getEmail() + "', Emailcount="
							+ user1.getEmailcount() + " where username='"
							+ user1.getUsername() + "';");
			preparedstatement1.execute();
			result1 = true;
			preparedstatement2 = conn
					.prepareStatement("update BankAccount set AccountNumber='"
							+ user1.getAccount() + "' where username='"
							+ user1.getUsername() + "';");
			result2 = preparedstatement2.execute();
			result2 = true;
	...
	public User getUser1(String userId) {
		...
		try {
			...
			ResultSet rs = preparedstatement1.executeQuery();
			preparedstatement2 = conn
					.prepareStatement("select * from BankAccount where username='"
							+ userId + "'");
			ResultSet rs1 = preparedstatement2.executeQuery();
	...
	public ArrayList<String> getUserNames() {
		...
		try {
			preparedstatement = conn.prepareStatement("use studbudctr;");
			preparedstatement.execute();
			preparedstatement1 = conn.prepareStatement("select * from user");
			ResultSet rs = preparedstatement1.executeQuery();
			while (rs.next()) {
